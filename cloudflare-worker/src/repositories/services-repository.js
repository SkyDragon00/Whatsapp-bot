...
export async function listServices(db, { includeDisabled = false, limit = 100 } = {}) {
	...
	const statement = includeDisabled
		? db.prepare('SELECT * FROM services ORDER BY enabled DESC, name COLLATE NOCASE LIMIT ?1').bind(limit)
		: db.prepare('SELECT * FROM services WHERE enabled = 1 ORDER BY name COLLATE NOCASE LIMIT ?1').bind(limit);
...
export async function createService(db, input) {
	const service = validateServiceInput(input);
	try {
		return await db
			.prepare(
				`INSERT INTO services (name, description, duration_minutes, price_cents, enabled)
				 VALUES (?1, ?2, ?3, ?4, ?5)
				 RETURNING *`,
			)
			.bind(service.name, service.description, service.duration_minutes, service.price_cents, service.enabled)
			.first();
	} catch (error) {
...
export async function updateService(db, id, input) {
	const serviceId = requirePositiveInteger(id, 'El servicio');
	...
	values.push(new Date().toISOString(), serviceId);
