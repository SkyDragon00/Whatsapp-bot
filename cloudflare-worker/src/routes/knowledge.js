...
export async function handleKnowledgeApi(request, env, url) {
	...
	if (match && request.method === 'DELETE') return jsonResponse(await deleteKnowledgeDocument(env.DB, match[1]));
	return null;
}
